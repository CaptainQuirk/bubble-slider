( function( $ ) {
                ...
                if ( !$( slider ).hasClass( 'in-progress' ) && !$( this ).hasClass( 'active' ) ) {
                    ...
                    $( slider ).find( '.bubble' ).each( function( i, bubble ) {
                        if ( $( bubble ).attr( 'bubble-position' ) > clickedBubbleNumber ) {
                            var sel = [
                                '.bubble[bubble-position=',
                                ( $( bubble ).attr( 'bubble-position' ) - 1 ),
                                ']'
                            ].join( '' );
                            events.push( {
                                bubble: $( bubble ),
                                position: $( slider ).find( sel ).position()
                            } );
                        }
                    } );

                    events.forEach( function( event ) {
                        $.fn.bubbleslider.moveToPosition(
                            $( slider ),
                            event.bubble,
                            event.position,
                            null,
                            function() {
                                event
                                    ...
                                    .attr(
                                        'bubble-position',
                                        event.bubble.attr( 'bubble-position' ) - 1
                                    );
                    ...
                    $.fn.bubbleslider.moveToPosition(
                        ...
                        function() {
                            ...
                            $.fn.bubbleslider
                                .settings
                                .clickedBubbleAfterAnimationCallback( $clickedBubble );
                    ...
                    $.fn.bubbleslider.moveToPosition(
                        ...
                        function() {
                            ...
                            $.fn.bubbleslider
                                .settings
                                .firstBubbleAfterAnimationCallback( $firstBubble );
                            $( slider ).removeClass( 'in-progress' );
                        }, 'down' );
                }
    ...
    $.fn.bubbleslider.defaults = {
        ...
        firstBubbleAfterAnimationCallback: function() {

        },
        clickedBubbleAfterAnimationCallback: function() {
     ...
     * @returns {undefined}
     */
    $.fn.bubbleslider.moveToPosition = function(
        $slider,
        $element,
        position,
        number,
        ...
        $element.animate( {
            ...
            progress: function( animation, progress ) {
                ...
                if ( null !== number ) {
                    switch ( direction ) {
                    case 'up' :
                        top = '-=' + $.fn.bubbleslider.computeSin( progress, 75 * number ); break;
                    case 'down' :
                        top = '+=' + $.fn.bubbleslider.computeSin( progress, 75 * number ); break;
                    }
